...
class predictor:
    def __init__(self):
        ...
        self.__last24_frame[0] = np.load(
            os.getcwd() + r'\Models\last24hours.npy')

        # Load models
        self.__model1 = keras.models.load_model(
            os.getcwd() + '\\Models\\model1.keras')
        self.__model2 = keras.models.load_model(
            os.getcwd() + '\\Models\\model2.keras')
        self.__model3 = keras.models.load_model(
            os.getcwd() + '\\Models\\model3.keras')

        # Load scalers
        self.__scalers = []
        for file in os.listdir(os.getcwd() + '\\Models\\Scalers'):
            self.__scalers.append(joblib.load(
                os.getcwd() + '\\Models\\Scalers\\' + file))
    ...
    def predict(self, model_number: int, inputs):
        ...
        for j in range(inputs.shape[0]):
            for i in range(7):
                inputs[j][i] = self.__scalers[i].transform(
                    inputs[j][i].reshape(-1, 1)).reshape(inputs.shape[2])
        if (model_number == 1):
            ...
            for i in range(7):
                predicted[i] = self.__scalers[i].inverse_transform(
                    predicted[i].reshape(-1, 1)).reshape(predicted.shape[1])
